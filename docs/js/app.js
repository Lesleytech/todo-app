...
function showTodos(todoList, isAdding) {
    ...
    if (todoList.length > 0) {
        todoListContainer.innerHTML = todoList.map((todo, index) => {
            return `<li id="${index}" class="todo-item ${todo.completed && `completed-task`}">
                        <span class="todo-task">${todo.task}</span>
                        <button class="done-btn" onclick="changeStatus(${index})">
                            ${todo.completed ? `<i class="fa fa-check-square-o" aria-hidden="true"></i>` : `<i class="fa fa-square-o" aria-hidden="true"></i>`}      
                        </button>
    ...
    isAdding && document.getElementById(`${todoList.length - 1}`).classList.add("added-transition");
}
...
function changeStatus(todoId) {
    const todos = JSON.parse(localStorage.getItem('todos'));
    const completedStatus = todos[todos.findIndex((todo, index) => index == todoId)].completed;
    todos[todos.findIndex((todo, index) => index == todoId)].completed = !completedStatus;
    localStorage.setItem('todos', JSON.stringify(todos));
    showTodos(todos);
}
